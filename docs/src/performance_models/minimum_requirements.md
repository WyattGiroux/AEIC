...
This section will vary from model to model and is where the actual performance data lives. While different performance models will store/parse data differently, in order to use the standard trajectory a model must provide a four-dimensional array of fuel flow rate in terms of altitude, true airspeed, rate of climb/descent, and aircraft mass.
